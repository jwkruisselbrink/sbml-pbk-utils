...
class PbkModelReportGenerator():
    ...
    def get_model_overview(self) -> pd.DataFrame:
        ...
        if model.isSetTimeUnits():
            dt.append(["Time resolution", self._get_unit_string(model.getTimeUnits()), "ok", ""])
        else:
            dt.append(["Time resolution", "", "error", "Time unit missing."])

        if model.isSetSubstanceUnits():
            dt.append(["Amounts unit", self._get_unit_string(model.getSubstanceUnits()), "ok", ""])
        else:
            dt.append(["Amounts unit", "", "error", "Substance unit missing."])

        if model.isSetVolumeUnits():
            dt.append(["Volume unit", self._get_unit_string(model.getVolumeUnits()), "ok", ""])
        else:
